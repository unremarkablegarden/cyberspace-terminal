...
const docs = await fs.promises.readdir(`${HOME}/bin/docs`)
ok('the four manual pages land', ['README.txt', 'API.txt', 'TUI.txt', 'NETWORK.txt'].every(f => docs.includes(f)))
...
const readme = `${HOME}/bin/docs/README.txt`
...
const api = await fs.promises.readFile(`${HOME}/bin/docs/API.txt`, 'utf8')
