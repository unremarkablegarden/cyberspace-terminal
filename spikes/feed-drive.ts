...
// --- a fresh run -------------------------------------------------------------
{
  ...
  m.tty.input('n'); await sleep(200)
  m.reset(); m.tty.input('\x0f'); await sleep(200)
  ok('^O asks before saving a note', m.screen().includes('SAVE NOTE'))
  m.tty.input('n'); await sleep(200)
  m.reset(); m.tty.input('\x0e'); await sleep(200)
  ok('^N still asks before saving a note', m.screen().includes('SAVE NOTE'))
  m.tty.input('n'); await sleep(200)
  m.tty.input('\x1b'); await sleep(200)
